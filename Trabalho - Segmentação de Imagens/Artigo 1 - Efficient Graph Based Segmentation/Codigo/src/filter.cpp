...
namespace Filter {
    ...
    public:
        ...
        void normalize() {
            ...
            for (int i = 0; i < length; i++) {
                sum -= 2 * std::abs(mask[0][i]);
                sum -= 2 * std::abs(mask[i][0]);
            }
            for (auto& row : mask) {
                for (auto& val : row) {
                    val /= sum;
    ...
    std::vector<float> smooth(const std::vector<float>& src, int width, int height, float sigma) {
    ...
    float diff(const std::vector<float>& r, const std::vector<float>& g, const std::vector<float>& b,
